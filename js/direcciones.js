direccionesModulo = (function () {
  ...
  function calcularYMostrarRutas () {
        ...
        let waypoints = [],
            puntosIntermedios = document.getElementById('puntosIntermedios');
        let coord

         for (  i = 0; i < puntosIntermedios.options.length; i++ ) {
          coord = puntosIntermedios.options[i].value.split(',', 2);
          coord = {lat: parseFloat(coord[0]), lng: parseFloat(coord[1])};
          waypoints.push({location: coord, stopover: false});
        }
        ...
        for (var i = 0; i < puntosIntermedios.length; i++) {
            ...
            marcadorModulo.agregarMarcadorRuta(puntosIntermedios[i].location, letter, false)
        }
        marcadorModulo.agregarMarcadorRuta(document.getElementById('hasta').value, String.fromCharCode('B'.charCodeAt(0) + puntosIntermedios.length), false)
         ...
         servicioDirecciones.route({
           ...
           if(status == "OK"){
            ...
            swal({
              text: "No pudimos calcular la ruta, por favor vuelva a probar con otra dirección",
              title: "¡Ups!",
              type: "error",
